"""Search MyCareersFuture (Singapore gov) jobs, translate JDs, and add to application pool."""
...
from prompts.system_prompts import MYCAREERSFUTURE_JD_SYSTEM
...
def _is_mcf_job_url(url: str) -> bool:
    return "mycareersfuture.gov.sg/job/" in url


def _search_mcf(keywords: str) -> list[dict]:
    """Search MyCareersFuture jobs via DuckDuckGo. Returns list of {title, url, snippet}."""
    query = f"site:mycareersfuture.gov.sg {keywords}"
    results = []
    try:
        with DDGS() as ddgs:
            for r in ddgs.text(query, max_results=MAX_RESULTS, timelimit="m"):
                url = r.get("href", "")
                if _is_mcf_job_url(url):
                    results.append({
                        "title": r.get("title", ""),
                        "url": url,
                        "snippet": r.get("body", ""),
...
def _try_fetch_page(url: str) -> str | None:
    """Try to fetch a MCF job page for fuller JD. Best-effort."""
    if not HAS_REQUESTS:
        return None
    try:
...
def _build_fallback_jobs(raw_jobs: list[dict]) -> list[dict]:
    ...
    for i, j in enumerate(raw_jobs):
        title = j.get("title", "")
        parts = [p.strip() for p in title.replace(" Jobs Singapore", "").replace(" | MyCareersFuture", "").split(" at ")]
...
def _translate_jobs(raw_jobs: list[dict]) -> list[dict]:
    ...
    lines = ["请分析以下 MyCareersFuture（新加坡政府职位平台）搜索结果片段，提取结构化信息并翻译为中文：\n"]
    for i, j in enumerate(raw_jobs):
    ...
    msg = "\n".join(lines)
    console.print("\n[bold]正在翻译和分析 MyCareersFuture 岗位信息...[/bold]\n")
    messages = [{"role": "user", "content": msg}]
    buffer: list[str] = []
    for token in stream_claude(messages, system=MYCAREERSFUTURE_JD_SYSTEM):
...
def run_mycareersfuture_jobs():
    """Entry point: search MCF jobs, translate, and optionally add to pool."""
    console.print(Panel.fit(
        "[bold cyan]🇸🇬 MyCareersFuture 岗位抓取[/bold cyan]\n"
        "[dim]搜索新加坡政府职位平台岗位 → 翻译为中文 → 加入投递池[/dim]",
        border_style="cyan",
    ))
    ...
    keywords = Prompt.ask("\n[cyan]请输入搜索关键词[/cyan]（如 product manager fintech）")
    if not keywords.strip():
        console.print("[yellow]关键词为空，已取消。[/yellow]")
        return

    console.print(f"\n[bold]🔎 正在搜索 MyCareersFuture 岗位:[/bold] {keywords}")
    raw_jobs = _search_mcf(keywords)
    if not raw_jobs:
        console.print("[yellow]未搜索到 MyCareersFuture 岗位结果。可能原因：DuckDuckGo 限流、关键词过于具体、或暂无相关岗位。[/yellow]")
        return

    console.print(f"[green]找到 {len(raw_jobs)} 个 MyCareersFuture 岗位[/green]")
    ...
    for idx in indices:
        ...
        records.append({
            ...
            "location": job.get("location", "") or "Singapore",
            "url": url,
            "salary": job.get("salary_estimate", ""),
            "status": "待投递",
            "applied_date": datetime.now().strftime("%Y.%m.%d"),
            "notes": f"MCF: {job.get('requirements_cn', '')[:200]}",
        })
